...
export default class MutualFundDetailComponent extends LightningElement {
    @api idFromTheParentLwc;
    @track record;
    @track navData;
    @track error;
    @track buyAmount = 0;
    ...
    @wire(getRecord, { recordId: '$idFromTheParentLwc', fields: FIELDS })
    ...
    fetchInvestedAmount() {
        getInvestedAmount({ mutualFundId: this.idFromTheParentLwc })
            .then(data => {
    ...
    handleBuy() {
        ...
        if (this.buyAmount > 0) {
            buyMutualFund({ mutualFundId: this.idFromTheParentLwc, investedAmount: this.buyAmount })
                .then(() => {
            ...
            console.log('Invalid'); // Debugging line
            this.showToast('Error', 'The amount should not be 0', 'error');
        }
    }
    ...
    handleAddToWatchlist() {
        addToWatchlist({ mutualFundId: this.idFromTheParentLwc })
            .then(() => {
